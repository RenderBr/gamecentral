...
<a title='User ID, not a placement.' style='text-decoration:none;' class='noselect mt-3 ms-1 gray'>#<?php echo $usersid; ?></a>
      ...
      $result42 = $conn->query($sql42);
            ...
            if($messageType == "text"){
              echo '<div class="d-flex bd-highlight bg-darkest" style="border-bottom: 1px solid #2F3133;"><div class="me-auto p-2"><a href="/user?u=' . $posterF . '" class="ms-2"><img class="icon-sm rounded-circle me-2" src="' . $avatarF . '">' . $posterF . ' <a class="gray noselect"> > </a> </a><i><a>' . $message . '</a></i></div><div class="p-2 gray noselect">' . $timeAgoF . '</div></div>';
            }
            if($messageType == "statusUpdate"){
              echo '<div class="d-flex bd-highlight bg-darkest" style="border-bottom: 1px solid #2F3133;"><div class="me-auto p-2"><a href="/user?u=' . $posterF . '" class="ms-2"><img class="icon-sm rounded-circle me-2" src="' . $avatarF . '">' . $posterF . ' <a class="noselect gray">is... </a> </a><i><a class="noselect">' . $message . '</a></i></div><div class="p-2 gray noselect sm-text">' . $timeAgoF . '</div></div>';
            }
          }
      }
      echo '</div>';
