...
<html lang="en">
	...
	<?php
		...
		include_once("modules/addKarma.php");
		$user = $_GET['u'];
		...
		if ($result->num_rows > 0) {
		  ...
		  while($row = $result->fetch_assoc()) {
			  ...
			  $getFriendCount = mysqli_query($conn, "SELECT * FROM friends WHERE friendCombo LIKE '%$user%' AND approved = 1");
				$friendCount = mysqli_num_rows($getFriendCount);
